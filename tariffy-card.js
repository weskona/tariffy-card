...
class TariffyCardEditor extends HTMLElement {
  ...
  _tariffyDevices() {
    ...
    entities.forEach(e => {
      ...
      result.push({
        id: e.device_id,
        name: devices[e.device_id]?.name || e.device_id,
      });
    });
  ...
  _syncValues() {
    const cfg = this._config;
    const sel = this.shadowRoot.querySelector('#device_id');
    const name = this.shadowRoot.querySelector('#name');
    const mode = this.shadowRoot.querySelector('#mode');
    if (sel) sel.value = cfg.device_id || '';
    if (name) name.value = cfg.name || '';
    if (mode) mode.value = cfg.mode || 'compact';
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        ha-select, ha-textfield { display: block; width: 100%; margin-bottom: 16px; }
        .hint { font-size: 12px; color: var(--secondary-text-color); margin: -10px 0 16px; }
      </style>
    ...
    requestAnimationFrame(() => {
      const deviceSel = this.shadowRoot.querySelector('#device_id');
      const nameInp = this.shadowRoot.querySelector('#name');
      const modeSel = this.shadowRoot.querySelector('#mode');

      if (deviceSel) {
        deviceSel.value = cfg.device_id || '';
        // ha-select fires 'selected' on mwc-list-item click
        deviceSel.addEventListener('selected', () => {
          const val = deviceSel.value;
          if (!val || val === this._config.device_id) return;
      ...
      if (modeSel) {
        ...
        modeSel.addEventListener('selected', () => {
          const val = modeSel.value;
          if (!val || val === this._config.mode) return;
          this._config = { ...this._config, mode: val };
          this._fire(this._config);
        });
        modeSel.addEventListener('value-changed', e => {
          const val = e.detail?.value;
          if (!val || val === this._config.mode) return;
          this._config = { ...this._config, mode: val };
          this._fire(this._config);
