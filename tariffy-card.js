class TariffyCard extends HTMLElement {
  ...
  _getDeviceName() {
    ...
    const device = (this._hass && this._hass.devices || {})[cfg.device_id];
...
class TariffyCardEditor extends HTMLElement {
  ...
  _devices() {
    ...
    Object.values(this._hass.entities || {}).forEach(e => {
      if (e.platform === 'tariffy' && e.device_id) tariffyDevices.add(e.device_id);
    });
  ...
  _syncValues() {
    const cfg = this._config;
    const deviceSel = this.shadowRoot.querySelector('ha-select#device_id');
    const nameInp = this.shadowRoot.querySelector('ha-textfield#name');
    const modeSel = this.shadowRoot.querySelector('ha-select#mode');
    if (deviceSel) deviceSel.value = cfg.device_id || '';
    if (nameInp) nameInp.value = cfg.name || '';
    if (modeSel) modeSel.value = cfg.mode || 'compact';
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        ha-select, ha-textfield { display: block; width: 100%; margin-bottom: 8px; }
        .hint { font-size: 12px; color: var(--secondary-text-color); padding: 0 0 8px 0; }
      </style>

      <div class="section">Gerät</div>

      <ha-select id="device_id" label="Tariffy-Gerät" naturalMenuWidth fixedMenuPosition>
        <mwc-list-item value="">— Gerät wählen —</mwc-list-item>
        ${devices.map(d => `<mwc-list-item value="${d.id}">${d.name}</mwc-list-item>`).join('')}
      </ha-select>
      <div class="hint">Alle Tariffy-Geräte werden automatisch erkannt</div>

      <ha-textfield id="name" label="Anzeigename (optional)" placeholder="Leer = Gerätename"></ha-textfield>

      <div class="section">Darstellung</div>

      <ha-select id="mode" label="Startmodus" naturalMenuWidth fixedMenuPosition>
        <mwc-list-item value="compact">Kompakt</mwc-list-item>
        <mwc-list-item value="detail">Detail</mwc-list-item>
      </ha-select>
    ...
    requestAnimationFrame(() => {
      ...
      if (deviceSel) {
        deviceSel.value = cfg.device_id || '';
        deviceSel.addEventListener('selected', e => {
          const val = e.detail?.value ?? deviceSel.value;
      ...
      if (modeSel) {
        modeSel.value = cfg.mode || 'compact';
        modeSel.addEventListener('selected', e => {
          const val = e.detail?.value ?? modeSel.value;
          if (val === undefined || val === null) return;
          this._config = { ...this._config, mode: val };
          this._fire(this._config);
        });
      }
    });
