...
class TariffyCardEditor extends HTMLElement {
  ...
  _syncValues() {
    ...
    if (picker) picker.value = cfg.device_id || cfg.entity_device_id || '';
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        ha-device-picker, ha-textfield, ha-select { display: block; width: 100%; margin-bottom: 16px; }
        .hint { font-size: 12px; color: var(--secondary-text-color); margin: -10px 0 16px 0; }
        .section { font-size: 12px; font-weight: 500; color: var(--secondary-text-color);
      ...
      <ha-device-picker
        label="Tariffy-Gerät"
      ></ha-device-picker>
      <div class="hint">Nur Tariffy-Geräte werden angezeigt</div>
    ...
    requestAnimationFrame(() => {
      ...
      if (picker) {
        ...
        picker.value = cfg.device_id || '';
        picker.integrationFilter = 'tariffy';
