...
class TariffyCardEditor extends HTMLElement {
  ...
  _syncValues() {
    ...
    const inp = this.shadowRoot.querySelector('#name');
    const mode = this.shadowRoot.querySelector('#mode');
    if (sel && document.activeElement !== sel) sel.value = cfg.device_id || '';
    if (inp && document.activeElement !== inp) inp.value = cfg.name || '';
    if (mode && document.activeElement !== mode) mode.value = cfg.mode || 'compact';
  }

  _updateDeviceOptions() {
    const sel = this.shadowRoot.querySelector('#device_id');
    if (!sel || document.activeElement === sel) return;
    const devices = this._devices();
    const current = sel.value;
    sel.innerHTML = `<option value="">— Gerät wählen —</option>` +
      devices.map(d => `<option value="${d.id}" ${current === d.id ? 'selected' : ''}>${d.name}</option>`).join('');
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      *{box-sizing:border-box;margin:0;padding:0}:host{display:block;padding:16px}
      .row{margin-bottom:16px}
      label{display:block;font-size:12px;color:var(--secondary-text-color);margin-bottom:4px;font-weight:500}
      select,input{width:100%;height:40px;padding:0 12px;border:1px solid var(--divider-color,rgba(0,0,0,.2));border-radius:8px;background:var(--card-background-color,#fff);color:var(--primary-text-color);font-size:14px;appearance:none;-webkit-appearance:none}
